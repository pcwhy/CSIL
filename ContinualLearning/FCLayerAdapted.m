classdef FCLayerAdapted < nnet.layer.Layer
    ...
    methods
        function layer = FCLayerAdapted(inputDim,outputDim,name,initialWeights) 
...
%             layer.Biases = dlarray(randn(outputDim,1).*0.0001);
            stdGlorot = sqrt(10/(inputDim + outputDim));
            layer.Weights = dlarray(rand(outputDim,inputDim).*stdGlorot);
            layer.Weights = layer.Weights - mean(layer.Weights(:));
