% Custom initial training loop & Custom continual learning loop
...
%     loss = crossentropy(squeeze(dlYPred),Y) + scalarL2Factor*penalty + fisherLoss;
    loss = crossentropy(stripdims(squeeze(dlYPred(:,:))),Y,'DataFormat','CB') ... 
        + scalarL2Factor*penalty + fisherLoss;
    gradients = dlgradient(loss, dlnet.Learnables);
end

function [gradients,state,loss,classificationLoss,trainDepth] = modelGradientsOnWeights(dlnet,dlX,Y)
