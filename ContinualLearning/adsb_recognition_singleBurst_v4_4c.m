...
% Controling direction of fingerprint cluster with a constant vector
...
if plots == "training-progress"
    ...
    figure(12);  
    lineRegionLossOfTrain = animatedline('Color','#D95319','lineWidth',1.5);
    ylim([-inf inf])
    xlabel("Iteration")
...
totalIters = 0;
abandonFlg = 0;
% regionVector = rand(1,featureDims);
regionVector = [1,zeros(1,featureDims-1)];
...
for epoch = 1:numEpochs
    ...
    for i = 1:numIterationsPerEpoch
        ...
        [gradients,state,loss,classificationLoss,trainDepth] = dlfeval(@modelGradientsOnWeights,dlnet,dlX,Yb,regionVector);
...
%             [prevSumDevationCosine,~] = calculateGrossMutualDistance(dlnet.Layers({dlnet.Layers.Name} == "Fingerprints").Weights);
            addpoints(lineRegionLossOfTrain,iteration,double(extractdata(trainDepth)));
...
% stem(sum(FPs./sqrt(sum(FPs.^2,2)).*regionVector./sqrt(sum(regionVector.^2,2)),2))
...
for epoch = 1:numEpochs
    ...
    for i = 1:numIterationsPerEpoch
        ...
        [gradients,state,loss,fisherLoss,trainDepth] = dlfeval(@modelGradientsOnWeightsEWC,dlnet,dlX, Yb,...
            prevFisherInfo0, prevDlnet, newClassesNum, 1, fingerprintLayerIdx, regionVector);                        
...
%             figure(statusFigureIdx);
            if mod(iteration,20) == 0 
                newCvAccuracy = cvAccuracy(dlnet, XTest, categorical(YTest),miniBatchSize,executionEnvironment,0);
                oldCvAccuracy = cvAccuracy(dlnet, cX, categorical(cY), miniBatchSize,executionEnvironment,0);
                [~,~,oldCVLoss] = dlfeval(@modelGradientsOnWeights,dlnet,prevCX,prevCY,regionVector);
...
newCvAccuracy = cvAccuracy(dlnet, XTest,categorical(YTest),miniBatchSize,executionEnvironment,0);
oldCvAccuracy = cvAccuracy(dlnet, cX, categorical(cY), miniBatchSize,executionEnvironment,0);
disp('CV accuracy aft. cont. learning');
[newCvAccuracy, oldCvAccuracy]
...
function [gradients,state,loss,fisherLoss,trainDepth] = modelGradientsOnWeightsEWC(dlnet, dlX, Y,...
    prevFisherInfo0, prevDlnet, newClassesNum, ewcLambda, fingerprintLayerIdx, regionVector)
    [dlYPred,state] = forward(dlnet,dlX);
    penalty = 0;
    scalarL2Factor = 0;
    if scalarL2Factor ~= 0
...
%     end
    ...
    FPs = dlnet.Learnables.Value(dlnet.Learnables.Layer == "Fingerprints");    
    regionLoss = mean(sum(FPs{:}(19:end,:)./sqrt(sum(FPs{:}(19:end,:).^2,2)).*regionVector./sqrt(sum(regionVector.^2,2)),2));
    
%     loss = crossentropy(squeeze(dlYPred),Y) + scalarL2Factor*penalty + fisherLoss;
    loss = 0.5.*regionLoss + crossentropy(stripdims(squeeze(dlYPred(:,:))),Y,'DataFormat','CB') ... 
        + scalarL2Factor*penalty + fisherLoss + sqrt(depth.^2);
    gradients = dlgradient(loss, dlnet.Learnables);
end

function [gradients,state,...
    loss,classificationLoss,trainDepth] = modelGradientsOnWeights(dlnet,dlX,Y,regionVector)
...
%     dlYPred = softmax(dlarray(squeeze(rawPredictions),'CB'));
    ...
    FPs = dlnet.Learnables.Value(dlnet.Learnables.Layer == "Fingerprints");    
    regionLoss = mean(sum(FPs{:}./sqrt(sum(FPs{:}.^2,2)).*regionVector./sqrt(sum(regionVector.^2,2)),2));
    trainDepth = regionLoss;
%     classificationLoss = crossentropy(stripdims(squeeze(dlYPred(:,:))),Y,'DataFormat','CB');
%     regionLoss = regionVector.*
    loss = classificationLoss + 0.5.*regionLoss;
