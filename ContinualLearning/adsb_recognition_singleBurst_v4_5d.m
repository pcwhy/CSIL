...
% expansion and manual spacing and knowledge distillation.
% fc_bf_fp is unlocked
...
%             gradientMasks{fingerprintLayerIdx-1,3} = {dlarray(FCLayerBiasMask)};
        ...
        [gradients,state,loss,fisherLoss] = dlfeval(@modelGradientsOnWeightsEWC,dlnet,dlX, Yb,...
            prevFisherInfo0, prevDlnet, newClassesNum, ewcLbd, fingerprintLayerIdx,prevClassNum);                        
...
function [gradients,state,loss,fisherLoss] = modelGradientsOnWeightsEWC(dlnet, dlX, Y,...
    ...
    [dlYPred,dlYPred2,bfFp,state] = forward(dlnet,dlX,'Outputs',{'softmax_1','Fingerprints','fc_bf_fp'});
    penalty = 0;
    scalarL2Factor = 0;
    if scalarL2Factor ~= 0
...
%     distilLoss = calcDistilLoss(squeeze(oldDlYPred),squeeze(dlYPred(:,:,1:18,:)),2);
    distilLoss = sum((squeeze(oldDlYPred)-squeeze(dlYPred2(1:prevClassNum,:))).^2,'all')./32;
    domainDiff = 0.*mean(sum(bfFp(1:prevClassNum,:)./sqrt(sum(bfFp(1:prevClassNum,:).^2,1)).*bfFp(prevClassNum + 1:end,:)./sqrt(sum(bfFp(prevClassNum + 1:end,:).^2,1)),1),'all');

    fisherLoss = calcFisherLoss(prevFisherInfo0, dlnet, prevDlnet, newClassesNum, fingerprintLayerIdx)*ewcLambda/2;
    loss = 0.1*domainDiff + 0.2.*distilLoss + crossentropy(squeeze(dlYPred),Y) + scalarL2Factor*penalty + fisherLoss;
