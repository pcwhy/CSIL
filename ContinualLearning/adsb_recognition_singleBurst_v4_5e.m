...
% expansion and manual zero padding and knowledge distillation and spacing loss.
% To test the trainability with channel-wise processing layer
clc;
close all;
clear;
rng default;
...
%     channelProcessingLayer(zbFeatureDims,'attn',[],[zbFeatureDims])
...
halfSimilarMatrix = zeros(size(weights,1), size(weights,1));
for i = 1:size(weights,1)
    ...
    for j = 1:size(weights,1)
        ...
        halfSimilarMatrix(i,j)=sum((nxtWeight./magNxtWeight)...
            .*(curWeight./magCurWeight));
    end
end
sumDeviationAngles = (sum(halfSimilarMatrix,'all') - sum(diag(halfSimilarMatrix)))./2
figure;imagesc(halfSimilarMatrix)
...
for i = 1:size(prevGradients0,1)
    ...
    prevGradients0{i,3}={gpuArray(rescale(prevGradients0{i,3}{:},refMin,refMax))};
end

for i = 1:size(prevFisherInfo0,1)
    prevFisherInfo0{i,3}={gpuArray(exp(prevGradients0{i,3}{:}.^2))};    
...
oldZbFeatureDims = zbFeatureDims;
newClassesNum = floor(size(unique(uY),1));
extendedDims = oldZbFeatureDims;
...
for i = 1:newClassesNum
    ...
    aUx_i = stripdims(squeeze(predict(dlnet,dlarray(ux_i,'SSCB'),'Outputs','fc_bf_fp')));
    unitAUx_i=(aUx_i./sqrt(sum(aUx_i.^2)))';
    fp_i = mean(aUx_i,2)';
    magFp_i = sqrt(sum(fp_i.^2));
    unitFp_i = fp_i ./ magFp_i;
    newFp = [0.*rand(1,oldZbFeatureDims), unitFp_i,];
...
concatFingerprints = gpuArray([[existingFingerprints,0.*rand(numOldClasses,extendedDims)]; newFingerprints]);
newFingerprintLayer = zeroBiasFCLayer(numClasses,numClasses+newClassesNum,'Fingerprints',concatFingerprints);
fingerprintMask = gpuArray([[ones(size(existingFingerprints)),zeros(numOldClasses,extendedDims)];...
    [zeros(newClassesNum,oldZbFeatureDims),ones(newClassesNum,extendedDims)]]);

priorLayerWeights = prevDlnet.Layers({prevDlnet.Layers.Name} == "fc_bf_fp").Weights;
priorLayerWeights  = ([priorLayerWeights;1e-4*rand(extendedDims,size(priorLayerWeights,2))]);
...
for epoch = 1:numEpochs
    ...
    for i = 1:numIterationsPerEpoch
        ...
        Xb = XTrain(:,:,:,idx);
        Yb = zeros(numClasses, miniBatchSize, 'single')+0.01;
        for c = 1:numClasses
            Yb(c,YTrain(idx)==(c)) = 1;
        end
...
%         learnRate = initialLearnRate;
        if isempty(velocities)
            ...
            for k = 1:fingerprintLayerIdx-2
                gradientMasks.Value{k}=gpuArray((zeros(size(gradientMasks.Value{k}))));
            end
            
            gradientMasks{fingerprintLayerIdx,3} = {gpuArray(fingerprintMask)};            
            gradientMasks{fingerprintLayerIdx-2,3} = {gpuArray(FCLayerWeightMask)};
%             gradientMasks{fingerprintLayerIdx-1,3} = {dlarray(FCLayerBiasMask)};
        ...
        [gradients,state,loss,fisherLoss] = dlfeval(@modelGradientsOnWeightsEWC,dlnet,dlX, Yb,...
            prevFisherInfo0, prevDlnet, newClassesNum, ewcLbd, fingerprintLayerIdx);                        
...
allCvAccuracy = cvAccuracy(dlnet, cat(4,XTest,cX),categorical(cat(1,YTest,cY)),miniBatchSize,executionEnvironment,1)
...
sumDeviationAnglesOld = sum(halfSimilarMatrix,'all')
maxOldSim = max(halfSimilarMatrix,[],'all')
imagesc(halfSimilarMatrix)
title("Lower similarity matrix of old fingerprints");

figure
weights = dlnet.Layers({dlnet.Layers.Name} == "Fingerprints").Weights;
weights = [weights(19:end,19:end)];
halfSimilarMatrix = zeros(size(weights,1), size(weights,1));
for i = 1:size(weights,1)
    curWeight = weights(i,:);  
    magCurWeight = sqrt(sum(curWeight.^2,2));
...
sumDeviationAnglesNew = sum(halfSimilarMatrix,'all')
maxNewSim = max(halfSimilarMatrix,[],'all')
imagesc(halfSimilarMatrix)
title("Lower similarity matrix of new fingerprints");

figure
weights = dlnet.Layers({dlnet.Layers.Name} == "Fingerprints").Weights;
weights = [weights(1:18,1:18);weights(19:end,19:end)];
halfSimilarMatrix = zeros(size(weights,1), size(weights,1));
for i = 1:size(weights,1)
    curWeight = weights(i,:);  
    magCurWeight = sqrt(sum(curWeight.^2,2));
...
sumDeviationAnglesPureAll = sum(halfSimilarMatrix,'all')
maxPureAllSim = max(halfSimilarMatrix,[],'all')
imagesc(halfSimilarMatrix)
title("Lower similarity matrix of all purified fingerprints");
...
selectors = extractdata(dlnet.Learnables.Value{9});
selectors = selectors(1:oldZbFeatureDims,:);
selectorCosSimMatrix = zeros(size(selectors,1),size(selectors,1));
selectorEucSimMatrix = zeros(size(selectors,1),size(selectors,1));
for i = 1:size(selectors,1)
    curSel = selectors(i,:);
    for j = 1:i-1
        nxtSel = selectors(j,:);
        selectorCosSimMatrix(i,j) = sum(curSel./vecnorm(curSel).*nxtSel./vecnorm(nxtSel),'all');
        selectorEucSimMatrix(i,j) = sqrt(sum((curSel-nxtSel).^2, 'all'));
    end
end
maxOldSelSim = max(selectorCosSimMatrix(:))

selectors = extractdata(dlnet.Learnables.Value{9});
selectors = selectors(oldZbFeatureDims + 1:end,:);
selectorCosSimMatrix = zeros(size(selectors,1),size(selectors,1));
selectorEucSimMatrix = zeros(size(selectors,1),size(selectors,1));
for i = 1:size(selectors,1)
    curSel = selectors(i,:);
...
maxNewSelSim = max(selectorCosSimMatrix(:))
...
figure;imagesc(selectorCosSimMatrix)
...
%     end
    ...
    accuracy = mean(YPred(:) == YTest(:));
    if confusionChartFlg == 1
        cm = confusionmat(categorical(YTest),YPred);
        cm = cm./sum(cm,2);
        figure
        imagesc(cm);
        figure
...
function [gradients,state,loss,fisherLoss] = modelGradientsOnWeightsEWC(dlnet, dlX, Y,...
    ...
    [selectorOut, oldDlYPred] = prevDlnet.predict(dlX,'Outputs',{'fc_bf_fp','Fingerprints'});
    [selectorOut2,dlYsoftmax,dlYPred2,state] = forward(dlnet,dlX,'Outputs',{'fc_bf_fp','softmax_1','Fingerprints'});
    penalty = 0;
    scalarL2Factor = 0;
    if scalarL2Factor ~= 0
...
%     conflict = (mean(dlYPred2(1:18,:),'all')-mean(dlYPred2(19:end,:),'all'))./10;
%     T = 2;
...
%     distilLoss = calcDistilLoss(squeeze(oldDlYPred),squeeze(dlYPred(:,:,1:18,:)),2);
    distilLossSel = 0.*mean((squeeze(selectorOut)-squeeze(selectorOut2(1:18,:))).^2,'all');
    distilLossFP = sum((squeeze(oldDlYPred)-squeeze(dlYPred2(1:18,:))).^2,'all')./32;
    distilLoss = distilLossSel + distilLossFP;
    fisherLoss = calcFisherLoss(prevFisherInfo0, dlnet, prevDlnet, newClassesNum, fingerprintLayerIdx)*ewcLambda/2;
    loss = 0.2.*distilLoss + crossentropy(squeeze(dlYsoftmax),Y) + scalarL2Factor*penalty + 1.*fisherLoss;
