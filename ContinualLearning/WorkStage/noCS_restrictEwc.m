...
if plots == "training-progress"
    ...
    grid on
    figure;
    lineDepth = animatedline('LineWidth',1);
    ylabel('Depth')
    ylim([-inf inf])
    xlabel("Iteration")
end
...
for epoch = 1:numEpochs
    ...
    for i = 1:numIterationsPerEpoch
        ...
        [gradients, state, loss, fisherLoss, depth] = dlfeval(@modelGradientsOnWeightsEWC, dlnet, dlX, Yb, ...
        ...
        if plots == "training-progress"
            ...
            addpoints(lineDepth, iteration, double(extractdata(depth)));
...
function [gradients,state,loss,fisherLoss,depth] = modelGradientsOnWeightsEWC(dlnet, dlX, Y,...
    ...
    prevSumDevLimit = prevClassNum*(prevClassNum-1)/2 * newAvgCosine;    
    
    fisherLoss = calcFisherLoss(prevFisherInfo0, dlnet, prevDlnet, newClassesNum, fingerprintLayerIdx)*ewcLambda/2;
%     loss = crossentropy(dlYPred,Y) + scalarL2Factor*penalty + fisherLoss + sqrt((trainDepth-prevSumDevLimit).^2);
