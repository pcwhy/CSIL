...
if skipDAGNet == 0
    ...
    numClasses = size(dlnet.Layers(13).Weights,1);    
    prevClassNum = numClasses;
end
...
fingerprintLayerIdx = 13;
learnableFpLayerIdx = 11;
...
for i = 1:newClassesNum
    ...
    newFp = [0.*rand(1,prevClassNum), unitFp_i(:,end-newClassesNum+1:end),];
...
concatFingerprints = gpuArray([...
    [existingFingerprints,0.*rand(prevClassNum,newClassesNum)]; ...
    newFingerprints]);
newFingerprintLayer = zeroBiasFCLayer(prevClassNum+newClassesNum,prevClassNum+newClassesNum,'Fingerprints',concatFingerprints);
fingerprintMask = gpuArray([[zeros(size(existingFingerprints)),zeros(prevClassNum,newClassesNum)];...
    [zeros(newClassesNum,prevClassNum),ones(newClassesNum,newClassesNum)]]);

priorLayerWeights = prevDlnet.Layers({prevDlnet.Layers.Name} == "fc_bf_fp").Weights;
priorLayerWeights  = ([priorLayerWeights;1e-4*rand(newClassesNum,size(priorLayerWeights,2))]);
% newFClayer = FCLayerAdapted(15680,zbFeatureDims, 'fc_bf_fp',priorLayerWeights);
newFClayer = fullyConnectedLayer(prevClassNum + newClassesNum,...
    'Name', 'fc_bf_fp', 'Weights', priorLayerWeights);
FCLayerWeightMask = [zeros(prevClassNum,size(priorLayerWeights,2));
    ones(newClassesNum,size(priorLayerWeights,2))];
FCLayerBiasMask = [zeros(prevClassNum,1);
    ones(newClassesNum,1)];
...
xNum = size(X,4);
rndSeq = randperm(xNum);
rndSeq = rndSeq(1:min(numel(rndSeq),5000));

continualLearnX = cat(4,continualLearnX,X(:,:,:,rndSeq));
continualLearnY = cat(2,continualLearnY,Y(rndSeq)');
...
%                 prevFisherInfo0, prevDlnet, newClassesNum, 0, fingerprintLayerIdx);            
%         else
%             [gradients,state,loss,fisherLoss] = dlfeval(@modelGradientsOnWeightsEWC,dlnet,dlX, Yb,...
%                 prevFisherInfo0, prevDlnet, newClassesNum, 1, fingerprintLayerIdx);                        
%         end
        [gradients,state,loss,fisherLoss] = dlfeval(@modelGradientsOnWeightsEWC,dlnet,dlX, Yb,...
            prevFisherInfo0, prevDlnet, newClassesNum, 1, learnableFPIdx,prevClassNum);                        
...
%             end
            ...
            % Let's lock some weights 
            for k = 1:learnableFpLayerIdx-3
                gradientMasks.Value{k}=gpuArray((zeros(size(gradientMasks.Value{k}))));
            end
            
            gradientMasks{learnableFpLayerIdx,3} = {gpuArray(fingerprintMask)};            
            gradientMasks{learnableFpLayerIdx-2,3} = {gpuArray(FCLayerWeightMask)};
            gradientMasks{learnableFpLayerIdx-1,3} = {dlarray(FCLayerBiasMask)};
...
% 
figure
weights = dlnet.Layers(13).Weights;
...
% 
% weights = dlnet.Layers(13).Weights(1:18,:);
% similarMatrix = zeros(size(weights,1), size(weights,1));
% for i = 1:size(weights,1)
%     curWeight = weights(i,:);  
%     magCurWeight = sqrt(sum(curWeight.^2,2));
...
%     end
% end
% sumDeviationAnglesOld = (sum(similarMatrix,'all') - sum(diag(similarMatrix)))./2
% 
% weights = dlnet.Layers(11).Weights(19:end,:);
% similarMatrix = zeros(size(weights,1), size(weights,1));
% for i = 1:size(weights,1)
%     curWeight = weights(i,:);  
%     magCurWeight = sqrt(sum(curWeight.^2,2));
...
%     end
% end
% sumDeviationAnglesNew = (sum(similarMatrix,'all') - sum(diag(similarMatrix)))./2
...
%     [dlYPred,state] = forward(dlnet,dlX);
    ...
    fisherLoss = 1.*calcFisherLoss(prevFisherInfo0, dlnet, prevDlnet, newClassesNum, fingerprintLayerIdx)*ewcLambda/2;
    distilLoss = sum((squeeze(oldDlYPred)-squeeze(dlYPred2(1:prevClassNum,:))).^2,'all')./32;

    loss = crossentropy(dlYPred,Y) + scalarL2Factor*penalty + fisherLoss + 0.2.*distilLoss;
