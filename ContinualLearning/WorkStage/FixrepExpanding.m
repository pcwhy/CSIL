...
concatFingerprints = [[existingFingerprints; newFingerprints],...
    0.001.*randn(prevClassNum+newClassesNum,newClassesNum)];
newFeatureEmbeddingLayer = fullyConnectedLayer(...
    prevClassNum+newClassesNum,'Weights',...
    [prevDlnet.Layers(12).Weights;...
    0.001.*randn(newClassesNum,size(prevDlnet.Layers(12).Weights,2))],...
    'Name','fc_bf_fp');

% newFingerprintLayer = FCLayer(numClasses,numClasses+newClassesNum,'Fingerprints',concatFingerprints);
newFingerprintLayer = zeroBiasFCLayer(numClasses+newClassesNum,...
    prevClassNum+newClassesNum,'Fingerprints',concatFingerprints);
...
% lgraph2 = lgraph2.removeLayers('classify_1');
lgraph2 = lgraph2.replaceLayer('Fingerprints', newFingerprintLayer);
lgraph2 = lgraph2.replaceLayer('fc_bf_fp', newFeatureEmbeddingLayer);
if skipDAGNet == 0
    lgraph2 = lgraph2.removeLayers('batchNorm_2');
    lgraph2 = lgraph2.connectLayers('fc_bf_fp','Fingerprints');
end
...
%         end
        [gradients,state,loss,fisherLoss] = dlfeval(@modelGradientsOnWeightsEWC,dlnet,dlX, Yb,...
            prevFisherInfo0, prevDlnet, newClassesNum, 0, fingerprintLayerIdx,prevClassNum);                        

        fisherLossLst(end+1)=(fisherLoss);
...
%             gradientMasks{fingerprintLayerIdx-1,3} = {dlarray([zeros(prevClassNum,20); ones(newClassesNum,20)])};
