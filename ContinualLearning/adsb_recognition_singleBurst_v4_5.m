% Matlab's initial training loop & Custom continual learning loop
% Without Dim Expansion with distill loss
...
%     yxBatchNorm('yxBatchnorm_1',featureDims)
...
function [gradients,state,loss,fisherLoss] = modelGradientsOnWeightsEWC(dlnet, dlX, Y,...
    ...
    loss = crossentropy(squeeze(dlYPred),Y) + scalarL2Factor*penalty + 1.*fisherLoss + 0.2.*distilLoss;
